...
namespace OrganizerApp.WebUI.Helpers.Api.OrganizerApp.Projects
{
    public class OrganizerAppProjectApiRequestHandler : IOrganizerAppProjectApiRequestHandler
    {
        ...
        {
            string doneProjectUri = ApiUriBuilder.DoneProjectUri.ToString();
            NameValueCollection parameters = new NameValueCollection()
            {
                { "id" , id.ToString() }
            };
            await _apiRequestHandler.ExecuteGetAsync(doneProjectUri, parameters);
        }

        public async Task Remove(int id)
        {
            string removeProjectUri = ApiUriBuilder.RemoveProjectUri.ToString();
            NameValueCollection parameters = new NameValueCollection()
            {
                { "id" , id.ToString() }
            };
            await _apiRequestHandler.ExecuteGetAsync(removeProjectUri, parameters);
        }
